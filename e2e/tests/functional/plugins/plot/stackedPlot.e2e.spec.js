...
test.describe('Stacked Plot', () => {
    ...
    test('Using the remove action removes the correct plot', async ({
        page
    }) => {
        ...
        await expect(
            page.locator('#inspector-elements-tree .js-elements-pool__item')
        ).toHaveCount(2);
    ...
    test('Can reorder Stacked Plot items', async ({ page }) => {
        ...
        const stackedPlotItem1 = page
            .locator('.c-plot--stacked-container')
            .nth(0);
        const stackedPlotItem2 = page
            .locator('.c-plot--stacked-container')
            .nth(1);
        const stackedPlotItem3 = page
            .locator('.c-plot--stacked-container')
            .nth(2);
        ...
        await expect(stackedPlotItem1).toHaveAttribute(
            'aria-label',
            `Stacked Plot Item ${swgA.name}`
        );
        await expect(stackedPlotItem2).toHaveAttribute(
            'aria-label',
            `Stacked Plot Item ${swgB.name}`
        );
        await expect(stackedPlotItem3).toHaveAttribute(
            'aria-label',
            `Stacked Plot Item ${swgC.name}`
        );
    ...
    test('Selecting a child plot while in browse and edit modes shows its properties in the inspector', async ({
        ...
        await page
            .locator(`[aria-label="Stacked Plot Item ${swgA.name}"] canvas`)
            .nth(1)
            .click();

        // Assert that the inspector shows the Y Axis properties for swgA
        await expect(
            page.locator('[aria-label="Plot Series Properties"] >> h2')
        ).toContainText('Plot Series');
        await expect(
            page.getByRole('list', { name: 'Y Axis Properties' }).locator('h2')
        ).toContainText('Y Axis');
        await expect(
            page.locator(
                '[aria-label="Plot Series Properties"] .c-object-label'
            )
        ).toContainText(swgA.name);

        // Click on the 2nd plot
        await page
            .locator(`[aria-label="Stacked Plot Item ${swgB.name}"] canvas`)
            .nth(1)
            .click();
        ...
        await page
            .locator(`[aria-label="Stacked Plot Item ${swgC.name}"] canvas`)
            .nth(1)
            .click();
        ...
        await page
            .locator(`[aria-label="Stacked Plot Item ${swgA.name}"]`)
            .click();
        ...
        await page
            .locator(`[aria-label="Stacked Plot Item ${swgB.name}"]`)
            .click();
        ...
        await page
            .locator(`[aria-label="Stacked Plot Item ${swgC.name}"]`)
            .click();

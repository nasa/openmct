...
test.describe('Operator Status', () => {
    ...
    test('operator status is visible and expands when clicked', async ({
        page
    }) => {
        await expect(
            page.locator('div[title="Set my operator status"]')
        ).toBeVisible();
    ...
    test('poll question indicator remains when blank poll set', async ({
        page
    }) => {
        await expect(
            page.locator('div[title="Set the current poll question"]')
        ).toBeVisible();
        await page
            .locator('div[title="Set the current poll question"]')
            .click();
        // set to blank
    ...
    test('operator status table reflects answered values', async ({ page }) => {
        ...
        const statusPollIndicator = page.locator(
            'div[title="Set my operator status"]'
        );
        ...
        const manageStatusPollIndicator = page.locator(
            'div[title="Set the current poll question"]'
        );
    ...
    test('clear poll button removes poll responses', async ({ page }) => {
        ...
        // clear the poll
        await page
            .locator('button[title="Clear the previous poll question"]')
            .click();
        ...
        expect(updatedRowValuesArr[COLUMN_STATUS_INDEX]).toEqual(
            UNSET_VALUE_LABEL
        );
    });

    test.fixme(
        'iterate through all possible response values',
        async ({ page }) => {

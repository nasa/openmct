...
test.describe.serial('Condition Set CRUD Operations on @localStorage', () => {
    test.beforeAll(async ({ browser }) => {
        ...
        await page
            .locator('li[role="menuitem"]:has-text("Condition Set")')
            .click();
        ...
        getConditionSetIdentifierFromUrl = conditionSetUrl
            .split('/')
            .pop()
            .split('?')[0];
        console.debug(
            'getConditionSetIdentifierFromUrl ' +
                getConditionSetIdentifierFromUrl
        );
        await page.close();
    });
    ...
    test('Condition set object properties persist in main view and inspector @localStorage', async ({
        ...
        expect
            .soft(page.locator('_vue=item.name=Unnamed Condition Set'))
            .toBeTruthy();
    ...
    test('condition set object can be modified on @localStorage', async ({
        page,
        openmctConfig
    }) => {
        ...
        // Click Edit Button
        await page
            .locator('text=Conditions View Snapshot >> button')
            .nth(3)
            .click();
        ...
        expect
            .soft(page.locator('text=Renamed Condition Set').nth(1))
            .toBeTruthy();
        // Verify Inspector Details has updated Name property
        expect
            .soft(page.locator('text=Renamed Condition Set').nth(2))
            .toBeTruthy();

        // Verify Tree reflects updated Name proprety
        // Expand Tree
        await page
            .locator(`text=Open MCT ${myItemsFolderName} >> span >> nth=3`)
            .click();
        // Verify Condition Set Object is renamed in Tree
        expect(
            page.locator('a:has-text("Renamed Condition Set")')
        ).toBeTruthy();
    ...
    test('condition set object can be deleted by Search Tree Actions menu on @localStorage', async ({
        ...
        await expect(
            page.locator(
                'a:has-text("Unnamed Condition Set Condition Set") >> nth=0'
            )
        ).toBeVisible();
        ...
        await page
            .locator(
                '[aria-label="OpenMCT Search"] >> text=Unnamed Condition Set'
            )
            .first()
            .click();
        ...
        expect(numberOfConditionSetsAtEnd).toEqual(
            numberOfConditionSetsToStart - 1
        );
...
test.describe('Basic Condition Set Use', () => {
    ...
    test('Can add a condition', async ({ page }) => {
        ...
        const numOfUnnamedConditions = await page
            .locator('text=Unnamed Condition')
            .count();
        expect(numOfUnnamedConditions).toEqual(1);
    });
    test('ConditionSet should display appropriate view options', async ({
        page
    }) => {
        ...
        await page
            .getByRole('listitem', { name: 'Save and Finish Editing' })
            .click();
        await page.click('button[title="Change the current view"]');

        await expect(
            page.getByRole('menuitem', { name: /Lad Table/ })
        ).toBeHidden();
        await expect(
            page.getByRole('menuitem', { name: /Conditions View/ })
        ).toBeVisible();
        await expect(
            page.getByRole('menuitem', { name: /Plot/ })
        ).toBeVisible();
        await expect(
            page.getByRole('menuitem', { name: /Telemetry Table/ })
        ).toBeVisible();
    });
});

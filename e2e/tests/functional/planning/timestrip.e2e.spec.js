...
test.describe('Time Strip', () => {
    test('Create two Time Strips, add a single Plan to both, and verify they can have separate Indepdenent Time Contexts @unstable', async ({
        ...
        const timestrip = await test.step('Create a Time Strip', async () => {
            const createdTimeStrip = await createDomainObjectWithDefaults(
                page,
                { type: 'Time Strip' }
            );
            const objectName = await page
        ...
        const plan =
            await test.step('Create a Plan and add it to the timestrip', async () => {
                ...
                await page.dragAndDrop(
                    `role=treeitem[name=/${createdPlan.name}/]`,
                    '.c-object-view'
                );
        ...
        await test.step('TimeStrip can use the Independent Time Conductor', async () => {
            ...
            const startBoundString = new Date(startBound)
                .toISOString()
                .replace('T', ' ');
            const endBoundString = new Date(endBound)
                .toISOString()
                .replace('T', ' ');

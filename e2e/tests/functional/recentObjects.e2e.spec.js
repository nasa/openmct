...
test.describe('Recent Objects', () => {
    ...
    test('Navigated objects show up in recents, object renames and deletions are reflected', async ({
        ...
        await page
            .getByRole('listitem', { name: folderA.name })
        ...
        expect(
            recentObjectsList.getByRole('listitem', { name: folderA.name })
        ).toBeTruthy();
        ...
        await expect(
            recentObjectsList.getByRole('listitem', { name: clock.name })
        ).toBeHidden();
    });
    ...
    test("Clicking on the 'target button' scrolls the object into view in the tree and highlights it", async ({
        ...
        await expect(clockTreeItem.locator('.c-tree__item')).not.toHaveClass(
            /is-targeted-item/
        );
    });
    ...
    test('Enforces a limit of 20 recent objects', async ({ page }) => {
        ...
        const lastFolderTreeItem = page
            .getByRole('tree', { name: 'Main Tree' })
            .getByRole('treeitem', {
                name: lastFolder.name,
                expanded: true
            });
        const lastClockTreeItem = page
            .getByRole('tree', { name: 'Main Tree' })
            .getByRole('treeitem', {
                name: lastClock.name
            });
        ...
        await page
            .getByRole('button', {
                name: `Open and scroll to ${lastClock.name}`
            })
            .click();

        // Assert that the Clock parent folder has expanded and the Clock is visible)
        await expect(
            lastFolderTreeItem.locator('.c-disclosure-triangle')
        ).toHaveClass(/--expanded/);
        await expect(lastClockTreeItem).toBeVisible();

        // Assert that the Clock treeitem is highlighted
        await expect(lastClockTreeItem.locator('.c-tree__item')).toHaveClass(
            /is-targeted-item/
        );
        ...
        // Assert that the Clock treeitem is no longer highlighted
        await expect(
            lastClockTreeItem.locator('.c-tree__item')
        ).not.toHaveClass(/is-targeted-item/);
    });
    ...
    function assertInitialRecentObjectsListState() {
        ...
        expect(
            recentObjectsList.getByRole('listitem').nth(0).getByText(clock.name)
        ).toBeTruthy();
        expect(
            recentObjectsList
                .getByRole('listitem', { name: clock.name })

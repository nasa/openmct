...
test.describe('AppActions', () => {
    test('createDomainObjectsWithDefaults', async ({ page }) => {
        ...
        await test.step('Create multiple flat objects in a row', async () => {
            ...
            await expect(page.locator('.l-browse-bar__object-name')).toHaveText(
                timer1.name
            );
        ...
        await test.step('Create multiple nested objects in a row', async () => {
            ...
            expect(folder2.url).toBe(
                `${e2eFolder.url}/${folder1.uuid}/${folder2.uuid}`
            );
            expect(folder3.url).toBe(
                `${e2eFolder.url}/${folder1.uuid}/${folder2.uuid}/${folder3.uuid}`
            );
        });
    });
    ...
    test('expandEntireTree', async ({ page }) => {
        ...
        const treePaneCollapsedItems = treePane.getByRole('treeitem', {
            expanded: false
        });
        ...
        const locatorTreeCollapsedItems = locatorTree.locator(

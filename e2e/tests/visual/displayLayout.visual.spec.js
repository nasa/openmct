...
test.describe('Visual - Display Layout', () => {
  test('Edit Marquee', async ({ page, theme, openmctConfig }) => {
    ...
    await swg1TreeItem.dragTo(layoutGridHolder, { targetPosition: { x: 0, y: 0 } });
    ...
    await editButton.click();
    await swg2TreeItem.dragTo(layoutGridHolder, { targetPosition: { x: 0, y: 0 } });
    await saveButton.click();
    await confirmSaveAndFinishEditingButton.click();
    ...
    await editButton.click();
    await child1LayoutTreeItem.dragTo(layoutGridHolder, { targetPosition: { x: 350, y: 0 } });

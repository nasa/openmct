...
describe('Smoke test suite', () => {
    ...
    it('new images are displayed when switching from fixed timespan to local clock', function () {
        cy.visit('/');
        cy.get('.c-create-button').click();
        cy.get('.c-menu').should('be.visible');
        cy.get('.icon-image').click();
        cy.get('.c-overlay__button-bar > .c-button--major').click();
        cy.get('.c-imagery__main-image__image').should('be.visible');
        cy.get('.c-conductor__mode-select > .c-menu-button > .c-button--menu').click();
        cy.get('.c-super-menu__menu > .icon-clock').click();
        cy.get('.c-imagery__main-image__image').should('be.visible'); //TODO with new assertion
    });
});

...
export default class AnnotationAPI extends EventEmitter {
    ...
    async getAnnotation(query, searchType) {
        ...
        const searchResults = (
            await Promise.all(
                this.openmct.objects.search(query, null, searchType)
            )
        ).flat();
        if (searchResults) {
    ...
    async addAnnotationTag(
        ...
        if (!existingAnnotation) {
            const targets = {};
            const targetKeyString = this.openmct.objects.makeKeyString(
                targetDomainObject.identifier
            );
            ...
            const newAnnotation = await this.create(
    ...
    removeAnnotationTag(existingAnnotation, tagToRemove) {
        if (
            existingAnnotation &&
            existingAnnotation.tags.includes(tagToRemove)
        ) {
            const cleanedArray = existingAnnotation.tags.filter(
                (extantTag) => extantTag !== tagToRemove
            );
            ...
            throw new Error(
                `Asked to remove tag (${tagToRemove}) that doesn't exist`,
                existingAnnotation
            );
        }
    }

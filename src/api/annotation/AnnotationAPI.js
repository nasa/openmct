...
export default class AnnotationAPI extends EventEmitter {
    ...
    constructor(openmct) {
        ...
        this.openmct.types.addType(ANNOTATION_TYPE, {
            ...
            initialize: function (domainObject) {
                ...
                domainObject.annotationType =
                    domainObject.annotationType || 'plotspatial';
            }
        });
    }
    /**
    ...
    async create({
        ...
        if (!Object.keys(targets).length) {
            throw new Error(
                `At least one target is required to create an annotation`
            );
        }

        if (!Object.keys(targetDomainObjects).length) {
            throw new Error(
                `At least one targetDomainObject is required to create an annotation`
            );
        }
        ...
        const originalContextPath =
            this.openmct.objects.getRelativePath(originalPathObjects);
    ...
    getAvailableTags() {
        if (this.availableTags) {
            const rearrangedToArray = Object.keys(this.availableTags).map(
                (tagKey) => {
                    return {
                        id: tagKey,
    ...
    async getAnnotations(domainObjectIdentifier) {
        const keyStringQuery = this.openmct.objects.makeKeyString(
    ...
    getTagsFromAnnotations(annotations, filterDuplicates = true) {
        ...
        if (filterDuplicates) {
            tagsFromAnnotations = tagsFromAnnotations.filter(
                (tag, index, tagArray) => {
        ...
        const fullTagModels =
            this.#addTagMetaInformationToTags(tagsFromAnnotations);
    ...
    #getMatchingTags(query) {
        ...
        const matchingTags = Object.keys(this.availableTags).filter(
            (tagKey) => {
                if (
                    this.availableTags[tagKey] &&
                    this.availableTags[tagKey].label
                ) {
                    return this.availableTags[tagKey].label
                        .toLowerCase()
    ...
    #addTagMetaInformationToResults(results, matchingTagKeys) {
        const tagsAddedToResults = results.map((result) => {
            const fullTagModels = this.#addTagMetaInformationToTags(
                result.tags
            );
            ...
            results.map(async (result) => {
                const targetModels = await Promise.all(
                    Object.keys(result.targets).map(async (targetID) => {
                        const targetModel = await this.openmct.objects.get(
                            targetID
                        );
                        const targetKeyString =
                            this.openmct.objects.makeKeyString(
                                targetModel.identifier
                            );
                        const originalPathObjects =
                            await this.openmct.objects.getOriginalPath(
                                targetKeyString
                            );
    ...
    #combineSameTargets(results) {
        ...
        results.forEach((currentAnnotation) => {
            const existingAnnotation = combinedResults.find(
                (annotationToFind) => {
                    return _.isEqual(
                        currentAnnotation.targets,
                        annotationToFind.targets
                    );
                }
            );
    ...
    #breakApartSeparateTargets(results) {
        ...
        results.forEach((result) => {
            Object.keys(result.targets).forEach((targetID) => {
                ...
                separatedResult.targetModels = result.targetModels.filter(
                    (targetModel) => {
                        const targetKeyString =
                            this.openmct.objects.makeKeyString(
                                targetModel.identifier
    ...
    async searchForTags(query, abortController) {
        ...
        const combinedSameTargets = this.#combineSameTargets(
            filteredDeletedResults
        );
        ...
        const resultsWithValidPath = appliedTargetsModels.filter((result) => {
            return this.openmct.objects.isReachable(
                result.targetModels?.[0]?.originalPath
            );
        });
        const breakApartSeparateTargets =
            this.#breakApartSeparateTargets(resultsWithValidPath);

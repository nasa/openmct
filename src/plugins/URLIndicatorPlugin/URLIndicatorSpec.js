...
define([
    ...
    './URLIndicatorPlugin',
    '../../MCT'
], function (testingUtils, URLIndicator, URLIndicatorPlugin, MCT) {
    describe('The URLIndicator', function () {
        ...
        describe('on initialization', function () {
            describe('with default options', function () {
                beforeEach(function () {
                    ...
                    indicatorElement =
                        openmct.indicators.add.calls.mostRecent().args[0]
                            .element;
                });

                it('has a default icon class if none supplied', function () {
                    expect(
                        indicatorElement.classList.contains('icon-chain-links')
                    ).toBe(true);
                });

                it('defaults to the URL if no label supplied', function () {
                    expect(
                        indicatorElement.textContent.indexOf(
                            pluginOptions.url
                        ) >= 0
                    ).toBe(true);
                });
            ...
            describe('with custom options', function () {
                ...
                it('uses the custom iconClass', function () {
                    expect(
                        indicatorElement.classList.contains('iconClass-checked')
                    ).toBe(true);
                });
        ...
        describe('when running', function () {
            beforeEach(function () {
                ...
                urlIndicator = URLIndicatorPlugin(pluginOptions)(openmct);
                indicatorElement =
                    openmct.indicators.add.calls.mostRecent().args[0].element;
            });
            ...
            it('indicates success if connection is nominal', async function () {
                ...
                expect(indicatorElement.classList.contains('s-status-on')).toBe(
                    true
                );
            });
            ...
            it('indicates an error when the server cannot be reached', async function () {
                ...
                await urlIndicator.fetchUrl();
                expect(
                    indicatorElement.classList.contains('s-status-warning-hi')
                ).toBe(true);
            });
        });
    });

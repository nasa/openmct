...
describe('the plugin', function () {
    ...
    describe('the conditionSet object', () => {
        it('is creatable', () => {
            expect(conditionSetDefinition.creatable).toEqual(
                mockConditionSetObject.creatable
            );
        });

        it('initializes with an empty composition list', () => {
            expect(
                mockConditionSetDomainObject.composition instanceof Array
            ).toBeTrue();
    ...
    describe('the condition set usage for condition widgets', () => {
        ...
        it('does not include the output label when the flag is disabled', () => {
            styleViewComponentObject.conditionSetDomainObject =
            ...
            return Vue.nextTick().then(() => {
                const hasNoOutput =
                    styleViewComponentObject.domainObject.configuration.objectStyles.styles.every(
                        (style) => {
                            return (
                                style.style.output === '' ||
                                style.style.output === undefined
                            );
                        }
                    );
        ...
        it('includes the output label when the flag is enabled', () => {
            ...
            return Vue.nextTick().then(() => {
                const outputs =
                    styleViewComponentObject.domainObject.configuration.objectStyles.styles.map(
                        (style) => {
    ...
    describe('the condition set usage for multiple display layout items', () => {
        ...
        it('updates applicable conditional styles', () => {
            ...
            styleViewComponentObject.initializeConditionalStyles();
            expect(styleViewComponentObject.conditionalStyles.length).toBe(2);
            styleViewComponentObject.updateConditionalStyle(
                conditionalStyle,
                'border'
            );
            ...
            return Vue.nextTick().then(() => {
                expect(
                    styleViewComponentObject.domainObject.configuration
                        .objectStyles
                ).toBeDefined();
                [boxLayoutItem, lineLayoutItem, notCreatableObjectItem].forEach(
                    (item) => {
                        const itemStyles =
                            styleViewComponentObject.domainObject.configuration
                                .objectStyles[item.id].styles;
                        expect(itemStyles.length).toBe(2);
                        const foundStyle = itemStyles.find((style) => {
                            return (
                                style.conditionId ===
                                conditionalStyle.conditionId
                            );
                        });
                        ...
                        const applicableStylesKeys = Object.keys(
                            applicableStyles
                        ...
                        Object.keys(foundStyle.style).forEach((key) => {
                            ...
                            expect(
                                applicableStylesKeys.indexOf(key)
                            ).toBeGreaterThan(-1);
                            expect(foundStyle.style[key]).toEqual(
                                conditionalStyle.style[key]
                            );
                        });
                    }
                );
            ...
            return Vue.nextTick().then(() => {
                ...
                [boxLayoutItem, lineLayoutItem, notCreatableObjectItem].forEach(
                    (item) => {
                        const itemStyle =
                            styleViewComponentObject.domainObject.configuration
                                .objectStyles[item.id].staticStyle;
                        ...
                        Object.keys(itemStyle.style).forEach((key) => {
                            ...
                            expect(itemStyle.style[key]).toEqual(
                                staticStyle.style[key]
                            );
                        });
                    }
                );
    ...
    describe('the condition check if old', () => {
                    ...
                    conditionCollection: [
                        {
                            ...
                            summary:
                                'Match if all criteria are met: Any telemetry is old after 5 seconds'
                        },
                        {
                            isDefault: true,
                            id: '2532d90a-e0d6-4935-b546-3123522da2de',
        ...
        it('should evaluate as old when telemetry is not received in the allotted time', (done) => {
            let conditionMgr = new ConditionManager(
                conditionSetDomainObject,
                openmct
            );
    ...
    describe('the condition evaluation', () => {
                    ...
                    conditionCollection: [
                        {
                            ...
                            summary:
                                'Match if all criteria are met: Any telemetry value is less than 0'
                        },
                        {
                            id: '39584410-cbf9-499e-96dc-76f27e69885d',
                            configuration: {
                            ...
                            summary:
                                'Match if all criteria are met: Any telemetry value is greater than 0'
                        },
                        {
                            ...
                            summary:
                                'Match if all criteria are met: Any telemetry value is greater than 1'
                        },
                        {
                            isDefault: true,
                            id: '2532d90a-e0d6-4935-b546-3123522da2de',
        ...
        it('should stop evaluating conditions when a condition evaluates to true', () => {
            ...
            let result = conditionMgr.conditions.map(
    ...
    describe('The Style Rule Manager', () => {
        it('should subscribe to the conditionSet after the editor saves', async () => {
            ...
            const styleRuleManger = new StyleRuleManager(
                stylesObject,
                openmct,
                null,
                true
            ...
            expect(
                styleRuleManger.subscribeToConditionSet

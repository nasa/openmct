...
define(['lodash', 'EventEmitter'], function (_, EventEmitter) {
    ...
    class TableRowCollection extends EventEmitter {
        ...
        sortAndMergeRows(rows) {
            ...
            if (
                this.firstRowInSortOrder(lastIncomingRow, firstExistingRow) ===
                lastIncomingRow
            ) {
                this.rows = [...sortedRowsToAdd, ...this.rows];
            } else if (
                this.firstRowInSortOrder(lastExistingRow, firstIncomingRow) ===
                lastExistingRow
            ) {
        ...
        mergeSortedRows(rows) {
            ...
            while (i < this.rows.length && j < rows.length) {
                ...
                if (
                    this.firstRowInSortOrder(existingRow, incomingRow) ===
                    existingRow
                ) {
        ...
        matchesFilters(row) {
            ...
            Object.keys(this.columnFilters).forEach((key) => {
                ...
                if (this.columnFilters[key] instanceof RegExp) {
                    doesMatchFilters =
                        this.columnFilters[key].test(formattedValue);
                } else {
                    doesMatchFilters =
                        formattedValue
                            .toLowerCase()
                            .indexOf(this.columnFilters[key]) !== -1;
                }
            });

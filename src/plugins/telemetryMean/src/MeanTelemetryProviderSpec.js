...
define(['./MeanTelemetryProvider', './MockTelemetryApi'], function (
    ...
    describe('The Mean Telemetry Provider', function () {
        ...
        it('supports telemetry-mean objects only', function () {
            const mockTelemetryMeanObject =
                mockObjectWithType('telemetry-mean');
            const mockOtherObject = mockObjectWithType('other');

            expect(
                meanTelemetryProvider.canProvideTelemetry(
                    mockTelemetryMeanObject
                )
            ).toBe(true);
            expect(
                meanTelemetryProvider.canProvideTelemetry(mockOtherObject)
            ).toBe(false);
        });
        ...
        describe('the subscribe function', function () {
            ...
            beforeEach(function () {
                subscriptionCallback = jasmine.createSpy(
            ...
            it('returns an average only when the sample size is reached', function () {
                ...
                setSampleSize(5);
                meanTelemetryProvider.subscribe(
                    mockDomainObject,
                    subscriptionCallback
                );
            ...
            it('correctly averages a sample of five values', function () {
                ...
                return waitForPromises()
                    .then(feedInputTelemetry.bind(this, inputTelemetry))
                    .then(
                        expectAveragesForTelemetry.bind(this, expectedAverages)
                    );
            });
            ...
            describe('given telemetry input with range values', function () {
                ...
                it("uses the 'rangeKey' input range, when it is the default, to calculate the average", function () {
                    ...
                    return waitForPromises()
                        .then(feedInputTelemetry.bind(this, inputTelemetry))
                        .then(
                            expectAveragesForTelemetry.bind(
                                this,
                                averageTelemetryForRangeKey
                            )
                        );
                });
            ...
            function expectObjectWasSubscribedTo(object) {
                return waitForPromises().then(function () {
                    expect(mockApi.telemetry.subscribe).toHaveBeenCalledWith(
                        object,
        ...
        describe('the request function', function () {
            it('requests telemetry for the associated object', function () {
                ...
                return meanTelemetryProvider
                    .request(mockDomainObject)
                    .then(function () {
                        expect(mockApi.telemetry.request).toHaveBeenCalledWith(
                            associatedObject,
                            undefined
                        );
                    });
            ...
            function whenTelemetryRequestedReturn(telemetry) {
                mockApi.telemetry.request.and.returnValue(
                    resolvePromiseWith(telemetry)
                );
            }
        });
        ...
        function createMockObjects() {
            ...
            associatedObject = {};
            mockApi.objects.get.and.returnValue(
                resolvePromiseWith(associatedObject)
            );
        }

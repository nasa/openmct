...
describe('the inspector', () => {
    ...
    it('should allow a style to be saved', () => {
        ...
        stylesViewComponent = createViewComponent(
            StylesView,
            selection,
            openmct
        );
        savedStylesViewComponent = createViewComponent(
            SavedStylesView,
            selection,
            openmct
        );

        expect(savedStylesViewComponent.$children[0].savedStyles.length).toBe(
            0
        );
    ...
    it('should display all saved styles', () => {
        ...
        return stylesViewComponent.$nextTick().then(() => {
            expect(savedStylesViewComponent.$children[0].$children.length).toBe(
                1
            );
        });
    });
    ...
    xit('should allow a saved style to be applied', () => {
        ...
        return stylesViewComponent.$nextTick().then(() => {
            const styleSelectorComponent =
                savedStylesViewComponent.$children[0].$children[0];

            styleSelectorComponent.selectStyle();

            return savedStylesViewComponent.$nextTick().then(() => {
                const styleEditorComponentIndex =
                    stylesViewComponent.$children[0].$children.length - 1;
                const styleEditorComponent =
                    stylesViewComponent.$children[0].$children[
                        styleEditorComponentIndex
                    ];
    ...
    it('should prevent a style from being saved when the number of saved styles is at the limit', () => {
        ...
        expect(
            SavedStylesView.methods.showLimitReachedDialog
    ...
    it('should allow styles from multi-selections to be saved', () => {
        ...
        return stylesViewComponent.$nextTick().then(() => {
            ...
            const saveStyleButtonIndex =
                styleEditorComponent.$children.length - 1;
            const saveStyleButton =
                styleEditorComponent.$children[saveStyleButtonIndex];
            ...
            expect(
                savedStylesViewComponent.$children[0].savedStyles.length
            ).toBe(1);
        });
    });
